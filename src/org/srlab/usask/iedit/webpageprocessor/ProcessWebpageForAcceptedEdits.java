...
class ExtractAcceptedEditData extends Thread{
	...
	public static void dataExtractor() {
		...
		try {
			...
			csvWriter = new CsvListWriter(new FileWriter("E:/Projects/SORejectedEdits/AUSE/Database/LargeScaleDataset/AcceptedEdits/Accepted_Edits_______.csv"),CsvPreference.STANDARD_PREFERENCE);
			...
			listReader = new CsvListReader(new FileReader("E:/Projects/SORejectedEdits/AUSE/Database/LargeScaleDataset/AcceptedEdits/SampledDatasetToDownloadWebpages/Sampled_Edits_"+year+".csv"), CsvPreference.STANDARD_PREFERENCE);
			...
			while((editList = listReader.read(processors))!= null) {
				...
				try {
					revisionGUID = editList.get(2).toString().trim();
				} catch (Exception e) {
					e.printStackTrace();
				}
								
				try {
					String fileLocation = "E:/Projects/SORejectedEdits/AUSE/Database/LargeScaleDataset/AcceptedEdits/Webpages/"+postId+".txt";
...
/************************ Previous Edit's Information  ********************************/
			...
			csvWriter.close();
			System.out.println("Total entry:"+ entryCounter);
